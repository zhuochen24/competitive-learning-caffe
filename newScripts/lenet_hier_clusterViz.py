...
for net_id, useNet in enumerate(netName):
	for exp_id in range(1,10):
		...
		caffemodel = caffe_dir+'examples/mnist/snapshot/{0}_{1}_{2}_iter_10000.caffemodel'.format(useNet,file_suffix,exp_id)
		if not os.path.isfile(caffemodel):
			print ' Missing file: ', caffemodel
			continue
		net = caffe.Net(caffe_dir+'examples/mnist/{0}_train_test.prototxt'.format(useNet), 
				caffemodel,
				caffe.TEST)
